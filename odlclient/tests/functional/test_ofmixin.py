...
from odlclient.tests.base import ApiTestCase
from odlclient.error import OpenflowProtocolError, VersionMismatch, NotFound
import odlclient.datatypes
...
class TestOfMixin10(ApiTestCase):
    ...
    def test_create_flow_multiple_actions(self):
        match = odlclient.datatypes.Match(eth_type="ipv4",
                                            ipv4_src="10.0.0.1",
                                            ipv4_dst="10.0.0.22",
                                            ip_proto="tcp",
                                            tcp_dst=80)
        output1to6 = odlclient.datatypes.Action(output=[1,2,3,4,5,6])
        flow = odlclient.datatypes.Flow(priority=12345, idle_timeout=30,
    ...
    def test_add_flow(self):
        match = odlclient.datatypes.Match(eth_type="ipv4",
                                            ...
                                            ip_proto="tcp",
                                            tcp_dst=80)
        output6 = odlclient.datatypes.Action(output=6)
        flow = odlclient.datatypes.Flow(priority=12345, idle_timeout=30,
